...
def train_model(target_variable, sheet_name):
    ...
    if not data.empty:
        ...
        if X_scaled is not None:
            ...
            plt.xlabel('Epoch')
            plt.legend(loc='upper right')
            plt.suptitle(f'LSTM Model - {target_variable} - {sheet_name}', fontsize=16)
            ...
            # Save the model
            model_save_path = os.path.join(output_folder, f'{target_variable}_LSTM_Model_{sheet_name}.h5')
