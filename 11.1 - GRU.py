...
def train_model(target_variable, sheet_name):
    ...
    if not data.empty:
        output_folder = 'C:\\omair\\Projects\\CapStone Project\\Crypto Project results\\GRU Model'
        ...
        if X_scaled is not None:
            ...
            reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=10, verbose=1, min_delta=0.0001,
                                          min_lr=1e-6)
            history = model.fit(X_train, y_train, epochs=500, batch_size=64, verbose=1, validation_data=(X_val, y_val),
            ...
            plt.xlabel('Epoch')
            plt.legend(loc='upper right')
            plt.suptitle(f'GRU Model - {target_variable} - {sheet_name}', fontsize=16)
            ...
            # Save the model
            model_save_path = os.path.join(output_folder, f'{target_variable}_GRU_Model_{sheet_name}.h5')
