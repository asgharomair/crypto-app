...
def build_cnn_model(input_dim):
    input_layer = Input(shape=(input_dim, 1))  # Reshape input to include the channel dimension
    ...
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.GlobalMaxPooling1D()(x)  # Use GlobalMaxPooling1D to avoid dimension issues
...
def train_model(target_variable, sheet_name):
    ...
    if not data.empty:
        ...
        if X_scaled is not None and y is not None:
            ...
            reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=5, verbose=1, min_delta=0.0001, min_lr=1e-6)
            ...
            plt.xlabel('Epoch')
            plt.legend(loc='upper right')
            plt.suptitle(f'CNN Model - {target_variable} - {sheet_name}', fontsize=16)
            ...
            # Save the model
            model_save_path = os.path.join(output_folder, f'{target_variable}_CNN_Model_{sheet_name}.h5')
            model.save(model_save_path)
            print(f"Model saved at {model_save_path}")
        else:
            print("Error in data preprocessing or empty dataset.")
    else:
